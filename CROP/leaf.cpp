...
//inline double Max(double a, double b) {return (__max(a,b));}
...
//Modified for SPUDSIM using Gandar and Tanner 1976 data in MPa based on current hourly psil and not predawn
{
	...
	if (predawn_psil/10. < -0.05) {
		effect=max((1+exp(rf_psil*rf_sensitivity))/(1+exp(rf_sensitivity*(rf_psil-predawn_psil/10.))),0.);
		psi_leafexpansion_stress = max((1 + exp(rf_psil * rf_sensitivity)) / (1 + exp(rf_sensitivity * (rf_psil - predawn_psil / 10.))), 0.);
	}
	else {
		effect = 1.;
