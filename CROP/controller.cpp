...
{
//	if !FLOAT_EQ(weather[iCur].time,0.0) return;
//	else
	...
	sprintf_s(DateForOutputBuff,"%.2d/%.2d/%4i",mm,id,iyyy);
	...
	if (initInfo.outtimeStep>1400) //24 hour output
	{
		...
		ostr << setiosflags(ios::right) 
			...
			<< setw(8) << setprecision(2) << plant->get_Pg2() << comma //for 24h output, gross photosynthetic rate in g plant-1 day-1
			//<< setw(8) << setprecision(2) << plant->get_Pg() << comma//for hourly output, gross photosynthetic rate in umol co2 m-2 ground s-1
			<< setw(8) << setprecision(3) << plant->get_Rg()+plant->get_Rm() << comma// for 24 hour output, total respiration in g plant-1 day-1
	...
	if (initInfo.outtimeStep>1400) //24 hour output
	{
		...
		ostr4 << setiosflags(ios::right) << setiosflags(ios::fixed)
			...
			<< setw(6) << weather[iCur].jday << comma
			<< setw(8) << setprecision(3) << int(weather[iCur].time * 24 + 0.1) << comma
			<< setw(8) << setprecision(2) << this->getPlant()->get_totalPlantNitrogen()*1000 << comma //total N content in plant, mg N plant-1
			<< setw(8) << setprecision(2) << this->getPlant()->get_totalLeafNitrogen()*1000 << comma// total leaf N,  mg N plant-1
			<< setw(8) << setprecision(2) << this->getPlant()->get_totalStemNitrogen()*1000 << comma// total stem N,  mg N plant-1
			<< setw(8) << setprecision(2) << this->getPlant()->get_totalRootNitrogen()*1000 << comma// total root N,  mg N plant-1
			<< setw(8) << setprecision(2) << this->getPlant()->get_totalTuberNitrogen()*1000 << comma// total tuber N, mg N plant-1
			<< setw(8) << setprecision(2) << this->getPlant()->get_totalDeadNitrogen()*1000 << comma // total dead N lost to senescecnce, mg N plant-1
			...
			<< setw(9) << setprecision(3) << this->getPlant()->get_CumulativeNitrogenSoilUptake()*1000 << comma//total N uptake, cumulative, mg N plant-1
			<< setw(9) << setprecision(3) << this->getPlant()->get_CumulativeNitrogenDemand()*1000 << comma//total N demand, cumulative, mg  plant-1
			<< setw(9) << setprecision(3) << this->getPlant()->get_seedNitrogenUsed()*1000 << comma// cumulative N translocated from seedpiece, mg g N plant-1
