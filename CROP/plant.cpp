...
void CPlant::calcGasExchange(const TWeather & weather, const TInitInfo info)
{
	...
	do {
		sunlit->SetVal(sunlitPFD(), info, NitrogenStatus.Nitrogendeficiencyone, weather.airT, weather.CO2, weather.RH,weather.wind, atmPressure, leafwidth, poolmod, PhotosyntheticFeedback, leafageEffect, weather.psil_, weather.ET_supply);
		shaded->SetVal(shadedPFD(), info, NitrogenStatus.Nitrogendeficiencyone, weather.airT, weather.CO2, weather.RH,weather.wind, atmPressure, leafwidth, poolmod, PhotosyntheticFeedback, leafageEffect, weather.psil_, weather.ET_supply);
		...
		if (develop->get_istage()!=1) //post TI
		{
			if (initInfo.bigleaf == 0)
			{
				...
				sunlit2->SetVal(sunlitPFD(), info, NitrogenStatus.Nitrogendeficiencyone, weather.airT, weather.CO2, weather.RH,weather.wind, atmPressure, leafwidth, 2., PhotosyntheticFeedback, leafageEffect, weather.psil_, weather.ET_supply);
				shaded2->SetVal(shadedPFD(), info, NitrogenStatus.Nitrogendeficiencyone, weather.airT, weather.CO2, weather.RH,weather.wind, atmPressure, leafwidth, 2., PhotosyntheticFeedback, leafageEffect, weather.psil_, weather.ET_supply);
				photosynthesis_gross = (sunlit->A_gross*sunlitLAI() + shaded->A_gross*shadedLAI())*(1-Tubmod)+Tubmod*(sunlit2->A_gross*sunlitLAI() + shaded2->A_gross*shadedLAI());
				photosynthesis_gross = (sunlit->A_gross*sunlitLAI() + shaded->A_gross*shadedLAI());
				if ((sunlitLAI()+shadedLAI())>0) reference = (sunlit->A_gross*sunlitLAI()*3./(sunlitLAI()+shadedLAI()) + shaded->A_gross*shadedLAI()*3./(sunlitLAI()+shadedLAI()))*(1-Tubmod)+Tubmod*(sunlit2->A_gross*sunlitLAI()*3./(sunlitLAI()+shadedLAI()) + shaded2->A_gross*shadedLAI()*3./(sunlitLAI()+shadedLAI()));
					else reference = 1.;
				reference = 1.;
				photosynthesis_net = (sunlit->A_net*sunlitLAI() + shaded->A_net*shadedLAI())*(1-Tubmod)+Tubmod*(sunlit2->A_net*sunlitLAI() + shaded2->A_net*shadedLAI());//
				photosynthesis_net = (sunlit->A_net*sunlitLAI() + shaded->A_net*shadedLAI());
				transpiration = (sunlit->ET*sunlitLAI() + shaded->ET*shadedLAI())*(1-Tubmod) + Tubmod*(sunlit2->ET*sunlitLAI() + shaded2->ET*shadedLAI());//mmol h2o m-2 ground s-1
				//transpiration = sunlit->ET; //mmol h20 m-2 leaf s-1
				//temperature = (sunlit->Tleaf*sunlitLAI() + shaded->Tleaf*shadedLAI())/LAI*(1-Tubmod)+Tubmod*(sunlit->Tleaf*sunlitLAI() + shaded->Tleaf*shadedLAI())/LAI;
				temperature = (sunlit->Tleaf*sunlitLAI() + shaded->Tleaf*shadedLAI())/LAI;
				set_sunPFD(sunlitPFD());
				set_shadePFD(shadedPFD());
				set_PFD(weather.PFD);
		...
		{
			photosynthesis_gross = (sunlit->A_gross*sunlitLAI() + shaded->A_gross*shadedLAI());//umol co2 m-2 ground s-1
			if ((sunlitLAI()+shadedLAI())>0) reference = (sunlit->A_gross*sunlitLAI()*3./(sunlitLAI()+shadedLAI()) + shaded->A_gross*shadedLAI()*3./(sunlitLAI()+shadedLAI()));
			else reference = 1.;
			photosynthesis_net = (sunlit->A_net*sunlitLAI() + shaded->A_net*shadedLAI());//
			...
			transpiration = (sunlit->ET*sunlitLAI() + shaded->ET*shadedLAI());//mmol h2o m-2 ground s-1
			if (LAI != 0) 
			{
				temperature = (sunlit->Tleaf*sunlitLAI() + shaded->Tleaf*shadedLAI())/LAI;
			}
...
//	set_absPAR(sunlitPFD()*.8+shadedPFD()*.8);
	set_sunPFD(sunlitPFD());
	set_shadePFD(shadedPFD());
	set_PFD(weather.PFD);
	set_SRAD(weather.solRad);
	set_sunLAI(sunlitLAI());
	set_shadeLAI(shadedLAI());
...
void CPlant::calcLeafArea(const TInitInfo info) 
{
	...
	if (info.bigleaf == 1 && !nodalUnit==NULL)
	{
