...
C
C        If(NShoot.ne.0) then wsun to remove the restriction otherwise there has mess for par output
	   If(NShoot.ne.0.and.LAI.gt.0.0) then !wsun add the restriction (when soil N is very low, there has NaN values)
C
C
